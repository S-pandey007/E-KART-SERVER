...
import { verifyGoogleIdToken } from "../../utils/google/googleClient.js";
...
const createSession = async (userId) => {
  try {
    ...
    logger.debug(
      `Calculating session expiry time ${process.env.REFRESH_EXPIRES_IN} days`
    );
    const days = parseInt(process.env.REFRESH_EXPIRES_IN);
    const expiresAt = new Date(Date.now() + days * 24 * 60 * 60 * 1000);
    logger.info(
      `Creating session for user: ${userId} with sessionId: ${sessionId}`
    );
...
const register = async ({
  ...
  try {
    ...
    if (!IsverifyOtp.success) {
      logger.warn(
        `OTP verification failed for ${email}: ${IsverifyOtp.message}`
      );
    ...
    // create session
    const { refreshToken, accessToken, sessionId } = await createSession(
      user._id
    );
...
const signInWithGoogle = async ( idToken ) => {
  try {
    if (!idToken) {
      return { success: false, message: "ID Token is required" };
    }
    // verify id token
    ...
    if (!email_verified) {
      return { success: false, message: "Email not verified by Google" };
    }
    ...
    if (user) {
      ...
      logger.info("User registered successfully through google:", email);
      // create session
      const { refreshToken, accessToken, sessionId } = await createSession(
        user._id
      );
...
const refreshAccessToken = async ({ refreshToken }) => {
  try {
    ...
    const accessToken = jwt.sign({ userId, sessionId }, process.env.JWT_KEY, {
...
const logout = async ({ userId, sessionId }) => {
  try {
    ...
    if (!session) {
      logger.warn(
        `No active session found for userId: ${userId} and sessionId: ${sessionId}`
      );
      return { success: false, message: "No active session found" };
    }
    logger.info(
      `UserId: ${userId} logged out successfully from sessionId: ${sessionId}`
    );
...
const resetPassword = async ({ email, newPassword }) => {
  try {
    ...
    if (!user.isActive || user.isBanned) {
      logger.warn(
        `resetPassword warning: User account is inactive or banned for email: ${email}`
      );
      return { success: false, message: "User account is inactive or banned" };
    }
...
export { register, login, refreshAccessToken, logout, resetPassword,signInWithGoogle };
